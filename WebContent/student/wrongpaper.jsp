	...
	<lable class="layui-btn layui-btn-sm layui-btn-normal">试卷属性：</lable><br>
		<div class="c-table c-table--border">
			<table cellspacing="0" cellpadding="0" border="0" role="c-table">
				...
				<td>${wquesList.get(0).tName}</td>
				<td >${wquesList.get(0).courName}</td>
				<td>${wquesList.get(0).className}</td>
				<td> ${wquesList.get(0).endDate}</td>
				<td> ${wquesList.get(0).testTime}min</td>
			</table>
		</div>
	</div>
	
	<div class="p20">
	<lable class="layui-btn layui-btn-sm layui-btn-normal">我的试卷：</lable><br>
		<div class="c-table c-table--border">
			<table cellspacing="0" cellpadding="0" border="0" role="c-table">
				<colgroup>
	              <col  width="100" >
				...
				<c:forEach var="wque" items="${wquesList}" varStatus="status">
					<tr>
						<td>单项选择</td>
						<td>${wque.queTitle}</td>
						<td class="layui-btn"> 正确答案：${wque.ans }</td>
					</tr>
				</c:forEach>
			</table>
		</div>
